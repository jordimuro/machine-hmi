...
function TagCard({ tag }) {
  ...
  const getValueColor = (name, value) => {
    ...
    // Add warning colors for temperature/pressure
  ...
  return (
    <div className="card relative">
      ...
      <div className="absolute top-4 right-4">
        <span className={`status-dot ${getQualityColor(tag.quality)}`} />
      </div>
      ...
      {/* Tag Name */}
      <div className="tag-label mb-2">{tag.name.replace(/([A-Z])/g, ' $1').trim()}</div>
...
export default function Dashboard() {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Process Overview</h2>
        <p className="text-gray-600">Real-time monitoring of machine parameters</p>
      </div>
      ...
      {tagArray.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="text-gray-500 text-sm mt-2">Check OPC UA connection</p>
        </div>
      ) : (
